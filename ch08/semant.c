...
static Tr_exp transDec(Tr_level level, S_table venv, S_table tenv, A_dec d) {
    switch(d->kind) {
        case A_varDec: {
            ...
            Tr_access m_access = Tr_allocLocal(level, TRUE); // to keep things easy, all vars are escaping
        ...
        case A_functionDec: {
            ...
            for(fun_list = d->u.function; fun_list; fun_list = fun_list->tail) {
                ...
                for(fl = fun_list->head->params; fl; fl = fl->tail) {
                    ...
                    if(m_head) {
                        m_tail->tail = U_BoolList(TRUE, NULL); // to keep things easy, vars are escaping
                        m_tail = m_tail->tail;
                    } else {
